...
Such rule should be based on the facts `Person` and `Greeting`. In particular, the Camel route will periodically add a `Person` fact into the remote 
knowledge base and retrieve a `Greeting` fact created by the rule.
...
combination used in the Decision Server xPaaS template must be used in the configuration of the current this quickstart.
